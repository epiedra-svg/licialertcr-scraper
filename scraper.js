...
(async () => {
  ...
  console.log("Esperando que el input exista en el DOM (sin visibilidad)...");
  await page.waitForSelector("#attr_cartelNm", {
    timeout: 60000,
    state: "attached"   // 🔥 NO visible, solo que exista
  });
  ...
  for (const palabra of keywords) {
    ...
    await page.locator("#attr_cartelNm").fill("", { force: true });

    // Escribir palabra (forzado)
    await page.locator("#attr_cartelNm").fill(palabra, { force: true });
